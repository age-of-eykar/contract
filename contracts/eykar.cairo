...
func mint{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(name: felt) {
    ...
    if (colony_id == 0) {
        ...
        world.write(
            x,
            y,
            Plot(owner=colony_id, structure=Structure.SETTLER_CAMP, availability=timestamp, stored=0),
        );
    }
...
func expand{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    let (timestamp) = get_block_timestamp();
    world.write(
        target_x, target_y, Plot(owner=colony_id, structure=2, availability=timestamp, stored=0)
    );
